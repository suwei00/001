<script setup lang='ts' name='transfer-in'>
...
<template>
  <div text-left>
    ...
    <p flex mb-23px>
      <label w-94px mr-16px>可转入金额:</label><span>¥ <jl-count :value="+(userBalance?.balance ?? 0)" /></span>
    </p>
    <!-- 转入金额 -->
    <div flex items-center mb-32px>
      <label mr-18px>转入金额:</label>
      <div relative mr-16px>
        <div>
          <el-input
            v-model="amount" style="width:320px;" placeholder="请输入转入金额"
      ...
      <label>今日剩余可转入次数：<span color-color-error font-semibold>5</span></label>
    </div>
    <!-- 快捷金额 -->
    <personal-tabs
    ...
    <jl-button lh-50px h-50px :disabled="disabled" :loading="loading" w-402px @click="run">
      确认转入
    </jl-button>
    <div mt-27px mb-9px>
      余额宝说明
