<script setup lang='ts' name="dialog-updateSecure">
...
const {
  isOpen, open, close,
  oldRef, oldShow,
  passwordRef, pwdShow, rePasswordRef, rPwdShow, showPwd, disabledBtn,
} = usePersonalDialog(form)

const { run: runUpdatePassword, loading } = useRequest(
  () => updatePassword({ old: form.old, password: form.password, ty: 2 }),
  {
    manual: true,
    onSuccess() {
...
// 校验
const isSecurePwdOld = computed(() => {
  if (!form.old)
    return true

  else
    return verifyCodeRegx.test(form.old)
})
...
// 按钮开关
watchEffect(() => {
  if (form.old && isSecurePwdOld.value && form.repassword && isSecurePwd.value && isPwdSame.value)
...
<template>
  <personal-dialog v-if="isOpen" title="修改安全密码">
    <template #right>
    ...
    <template #body>
      <div class="item">
        ...
        <input
          ...
          v-model="form.old"
          type="password"
          placeholder="请输入旧的6位数字安全密码"
          autocomplete="new-password"
          maxlength="6"
          @input="form.old = form.old.replace(/[^0-9]/g, '')"
        >
        <span v-show="!isSecurePwdOld" class="error">安全密码仅限6位数字</span>
        <div class="icon" @click="showPwd('old')">
          <img v-if="oldShow" src="@icons/personal/finance/eye-open.svg">
      ...
      <div class="forget">
        <span @click="goForget">忘记安全密码？</span>
      </div>
    </template>
  </personal-dialog>
</template>
