<script setup lang='ts' name="byPhone">
...
const { run: runCheckPhone } = useRequest(() => checkMemberPhone({ username: username.value, phone: `${phone.value}` }), {
  manual: true,
  onSuccess(v) {
    if (v)
      step.value = 2
  ...
  <div class="byPhone">
    ...
    <div v-if="step === 1" class="form">
      <div class="form-i">
        <label>账号</label>
        <input
          v-model="username" type="text" placeholder="请输入您的账号" maxlength="14"
        >
      </div>
      <div class="form-i">
        <label>手机号码</label>
        <input
          v-model="phone" type="tel" placeholder="请输入您的手机号码" maxlength="11"
          @input="phone = phone.replace(/[^0-9]/g, '')"
        >
      </div>
      <button class="create" :disabled="disabledBtn1" :class="{ dis: disabledBtn1 }" @click="runCheckPhone">
        下一步
      </button>
    </div>
    ...
    <div v-else-if="step === 2" class="form">
      <div class="form-i">
        ...
        <input
          ...
          maxlength="20"
          @input="password = password.replace(/[^0-9a-zA-Z]/g, '')"
          @focus="pwdEye = true"
          @blur="pwdEye = false"
        >
      ...
      <div class="form-i">
        ...
        <input
          ...
          maxlength="20"
          @input="repassword = repassword.replace(/[^0-9a-zA-Z]/g, '')"
          @focus="rPwdEye = true"
          @blur="rPwdEye = false"
        >
