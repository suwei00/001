...
import bgSrc3 from '~/assets/images/noData/trade.png?preset=full'
...
const flagsFilter2 = (item: any) => {
  ...
  if (item.done_issue !== item.total_issue)
    obj.text = `${item.done_issue}/${item.total_issue}`
...
const onSearch = () => {
  router.replace({
    path: `/my/lottery-fine/${params.value.pageType}`,
    query: {
      ...route.query,
    },
  })
...
<template>
  <div class="tradeFineWarp">
    ...
    <div class="dataContentWarp">
      <CustomizePullDownUp
        ref="customizePullDownUp" v-model="setParam" :list="list" :no-data-img="bgSrc3" :first-down-load="true"
        @init="init"
      >
        <template #bSContent>
          <dl class="dataContent">
            <template v-for="(item, index) in list" :key="index">
              <dd>
                <div
                  class="ddMain"
                  @click="goTo('/my/lottery-fine/lottery-fine-details', { pageType: String(params.pageType), ...item })"
                >
                  <div class="ddContent">
