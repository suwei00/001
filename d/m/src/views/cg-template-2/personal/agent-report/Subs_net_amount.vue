...
const goToSubItem = (item: any) => {
  dataListDetail.value = {
    ...
    ty: curDate.value.ty // formData.value.ty,
  };
  router.push({
    path: `/personal/agent-report/detail`,
    query: {
      ...dataListDetail.value,
      ...curDate.value
    }
...
<template>
  <div class="sub">
    ...
    <CgTable ref="cgTableRef" @sort="sort" :form-data="{ ...formData, ...curDate }" :columns="columns" :list-api="APIs.getAgencySub">
      <template #company_net_amount="{ row }">
        <cg-amount :amount="row.company_net_amount" :segmentation="true" suffix="K" padding="0" :precision="2"></cg-amount>
      </template>
