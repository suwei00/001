...
// 添加银行卡
export const insertBank = (data: any) =>
...
// 银行卡维护列表
export const getBanksMaintainList = (params: { id: string }) =>
  request<{ [text: string]: string }>({ url: 'member/withdraw/bank/check', params });
...
export const withdraw = (data: {
  ...
  ts?: string;
  verify_code?: string;
}) => request({ url: 'finance/withdraw', method: 'post', data });

// 获取短信验证码
export const getPhoneCode = (data: { flag: string; ty: number | string }) =>
  request({ url: 'sms/online', method: 'post', data });
...
// 读取站内信
export const readUserMsg = (data: { id: string }) =>
  request({ url: 'member/message/read', method: 'post', data });
// 读取公告
export const readNotices = (data: { id: string }) =>
  request({ url: 'member/notices/read', method: 'post', data });
// 删除站内信 1 精确删除 2 删除所有已读 3删除所有站内信
export const deleteUserMsg = (data: { flag?: number; ids?: string }) =>
  request({ url: 'member/message/delete', method: 'post', data });
...
// 离线支付生成二维码
export const getOfflinePayQrCode = (data: { bid: string; amount: number; code: string }) =>
  request<{ qr: string }>({ url: '/finance/gen/code', method: 'post', data });
...
// 删除推广链接
export const memberLinkDelete = (params: { id: string }) =>
  request({ url: 'member/link/delete', params });

// 添加推广链接
export const memberLinkInsert = (data: any) =>
...
// 获取场馆根据id
export const getVenueById = (params: { pid: string; page: number; page_size: number }) =>
  request({ url: 'member/slot/list', params });
...
// 根据场馆ID获取余额
export const getBalanceByVenueId = (id: string) =>
  request<string>({ url: `plat/balance/${id}`, noToast: true });
...
export const transfer = (data: { pid: string; ty: 1 | 2, amount?: number }) =>
  request<string>({ url: 'plat/transfer', method: 'post', data });
// 一键回收时不要Toast
export const transferAll = (data: { pid: string; ty: 1 | 2 }) =>
  request<string>({ url: 'plat/transfer', method: 'post', data, noToast: true });

// 设置假免转
export const setAutomaticTransfer = (params: { state: 1 | 2 }) =>
  request({ url: 'member/automatic/set', params });

// 编辑头像
export const updateAvatar = (params: { id: string }) =>
  request<string>({ url: 'member/avatar/update', method: 'get', params });

// 个人交易记录
export const getTradeRecord = (params: {
  flag: 271 | 272 | 273 | 276 | 274 | 275 | 278;
  page: number;
  page_size: number;
  date_flag: '1' | '2' | '3' | '4' | '5' | '6';
}) => request<any>({ url: 'member/record/trade', params });

// 下级报表
export const getUnderReport = (params: {
  player_name?: string;
  page: number;
  page_size: number;
  ty: '1' | '2' | '3' | '4' | '5' | '6';
...
export const getMemberList = (params: {
  ...
  username?: string;
  parent_name?: string;
}) => request<any>({ url: 'member/list', params });
...
// 绑定zalo号
export const memberZalo = (data: { zalo: string }) =>
  request<string>({ url: 'member/bindzalo', method: 'post', data });
// 我的盈利/代理报表
export const getMyProfit = (data: { username?: string; ty: DateFlag }) =>
  request<{
    bet_amount: number;
    deposit: number;
    withdraw: number;
...
// 在线邮箱验证码
export const mailOnline = (data: { mail: string; ty: string }) =>
  request<string>({ url: 'mail/online', method: 'post', data });

// 绑定邮箱
export const setBildEmail = (data: { email: string; code: string; sid: string; ts: string }) =>
  request<string>({ url: 'member/bindemail', method: 'post', data });

// 手机验证码
export const smsOnlineCode = (data: { flag: string; ty: string; tel?: string }) =>
  request<string>({ url: 'sms/online', method: 'post', data });
...
export const getAgencySub = (params: {
  page: number;
  page_size: number;
  ty: DateFlag;
  flag: 5 | 6 | 7 | 8 | 1 | 3 | 2 | 4 | 9 | 10 | 11 | 13 | 14;
  username: string;
}) => request<{ bet_mem_count: number }>({ url: 'member/agency/report/sub', params });

// 获取banner flags 1
export const getBannerByFlags = (params: { flags: number; device?: number }) =>
  request({ url: 'member/banner', params });

// 绑定邮寄地址
export const bindAddress = (data: { address: string }) =>
  request({ url: 'member/update', method: 'post', data });

// 保存生日
export const updateBirth = (data: { birth: string }) =>
  request({ url: 'member/update', method: 'post', data });
...
// 用户名重复
export const checkUsernameAvailable = (params: { username: string }) =>
  request({ url: 'member/available', params });

// 发送验证码
export const sendOfflineSms = (data: { username: string; tel: string; flag: string; ty: 1 | 2 }) =>
  request({ url: 'sms/offline', method: 'post', data });
...
// 活动申请
export const getPromotionApply = (data: any) =>
...
// 场馆检查是否维护和定时维护
export const getPlatformCheck = (data: { id: string; rflag?: number }) =>
  request({ url: 'member/platform/check', method: 'post', data });

// 奖池金额
export const getjackpot = () => request({ url: 'member/slot/bonus', method: 'get' });

export const getPlatLaunch = (params: { id: string; code?: string }) =>
  request<string>({ url: `plat/launch/${params.id}`, params });
// 检查手机号
export const checkMemberPhone = (data: { username: string; phone: string }) =>
  request({ url: 'member/check/phone', method: 'post', data });
...
// 下级明细场馆输赢
export const getReportSubPlat = (params: { username?: string; ty?: string }) =>
  request<any>({ url: 'member/agency/report/sub/plat', params });
// 下级明细场馆余额
export const getAgencyReportSubBalance = (params: { username?: string; ty?: string }) =>
  request<any>({ url: 'member/agency/report/sub/balance', params });
// 个人交易明细
export const getTradeListDetail = (params: { id: string; flag: string }) =>
  request<any>({ url: 'member/record/tradedetail', params });
...
export const gamesearch = (data: {
  ...
  page_size: number;
  flag: number;
}) => request({ url: 'member/slot/search', method: 'post', data });

// 获取app下载地址
export const getAppDownloadUrl = (params: { dv: number }) =>
  request<{
    platform: string;
    version: string;
    is_force: number;
    content: string;
    url: string;
  }>({ url: 'member/app/upgrade', params });

// 绑定手机
export const setBindPhone = (data: {
  phone: string;
  verify_code: string;
  sid: string;
  ts: string;
}) => request({ url: 'member/bindphone', data, method: 'post' });
...
export const getAdvertise = () =>
  ...
  }>({ url: 'member/banner/web', params: { flags: 5 } });
...
export const getFlowDetail = (params: {
  page?: number;
  page_size?: number;
}) => request<any>({ url: 'promo/flow/detail', params });
// 代理优惠
export const getVipConfit = () => request({ url: 'member/vip/config' });
