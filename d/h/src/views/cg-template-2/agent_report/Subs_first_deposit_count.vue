...
const { option: agentFirstDepositCountSort } = useOption('agentFirstDepositCountSort');
...
const columns = ref([
    ...
    {
        title: 'Số Tiền<br/>Nạp',
        field: 'first_deposit_amount',
    },
    {
        title: 'Thời Gian<br/>Nạp Tiền',
    ...
    <div class="sub cg-list-container">
        <div class="input-container">
            <div class="time-sort">
                ...
                <div class="sort">
                    <cg-select rowKey="value" showLabelKey="label" :data="agentFirstDepositCountSort"
                        v-model="formData.sort" selectLeftText="HỦY BỎ" selectRightText="XÁC NHẬN"
                        @on-ok="onSort">
                        <template #item="{ data }">
                            <div>
                                {{ data.label }}
        ...
        <CgList ref="listRef" :list-api="APIs.getAgencySub" :form-data="tableData">
            <template #default="{ data }">
                <CgTable :columns="columns" :table-data="data">
                    <template #first_deposit_amount="{ row }">
                        <div class="center">
                            <cg-amount :amount="row.first_deposit_amount" :size="26" :segmentation="true" suffix="K"
                                padding="0"></cg-amount>
                        </div>
                    </template>
                    <template #first_deposit_at="{ row }">
