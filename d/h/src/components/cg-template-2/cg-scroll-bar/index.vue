  ...
  const timer = setTimeout(() => {
    ...
    if (props.always) {
      ...
      contentBoxRef.value?.addEventListener('scroll', e => {
        barMove.value =
          ((e.target as HTMLElement).scrollTop / contentBoxRef.value?.clientHeight!) * 100;
      });
    } else {
      // 需要滚动条消失
      let startTop: number;
...
<template>
  <div
    :style="{ height: `${height}` }"
    :class="{ 'cg-scroll-bar-inside': inside }"
    class="cg-scroll-bar"
  >
    <div
      ref="contentBoxRef"
      id="contentBoxID"
      :style="{ maxHeight: maxHeight && `var(--cg-px-${maxHeight})` }"
      class="content-box"
    >
    ...
    <template v-if="always">
      <div
        v-show="showBar"
        class="bar-box"
        :class="{ 'yellow-style': yellowStyle, inside }"
        :style="{
      ...
      >
        <div
          :style="{
            height: `${barHeight >= 100 ? 0 : barHeight}%`,
            transform: `translateY(${barMove}%)`,
    ...
    <template v-else>
      ...
      >
        <div
          v-show="showBar"
          class="bar-box"
          :class="{ 'yellow-style': yellowStyle }"
          :style="{
            backgroundColor: background,
            height: `${scrollBarHeight}%`,
            top: `${scrollBarTop}%`
